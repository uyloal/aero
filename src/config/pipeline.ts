import type { InlineRule } from '../mihomo/types'
...
export interface CategoryConfig {
  sources: readonly Upstream[]
  customRules?: readonly InlineRule[]
}
...
/**
 * Pipeline 业务分类、远端 URL 列表及可选的自定义规则
 * 使用 `as const satisfies Record<string, CategoryConfig>` 保留字面量联合类型
 */
export const PIPELINE_CONFIG = {
  ...
  sg: {
    sources: [{ id: 'bm7-telegram-sg', url: bm7('TelegramSG') }]
  },
  us: {
    sources: [
  ...
  eu: {
    sources: [{ id: 'bm7-telegram-nl', url: bm7('TelegramNL') }]
  }
